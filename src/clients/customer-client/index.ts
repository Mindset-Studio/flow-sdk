import { Filter, ListResponse } from "../../types";
import BaseClient from "../base-client/base";
import { Payment } from "../invoice-client/types";
import { BatchChargeCustomersProps, BatchResponse, BatchStatus, ChargeCustomerBaseProps, ChargeCustomerProps, Customer, CustomerProps, EditCustomerProps, RegisterProps, RegisterResponse, RegisterStatus } from "./types"
...
export default class FlowCustomerClient extends BaseClient {
    ...
    async chargeCustomer(props:ChargeCustomerProps){
        const signature = this.signParams({...props, apiKey:this.apiKey})
        const body = this.generateSearchParams({...props, s:signature, apiKey:this.apiKey})
